...
async def reliability_middleware(request: Request, call_next):
    ...
    try:
        ...
        logger.exception("Unhandled API error request_id=%s method=%s path=%s error=%s", request_id, request.method, request.url.path, type(exc).__name__)
...
# Phase 7: serve the admin dashboard from the same origin as the API.
